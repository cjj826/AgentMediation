...
你是{self.name}。请你对**你刚才的发言**进行**自我反思和评估**，请依次检查是否存在以下问题，这些问题可能会严重削弱发言在真实调解场景中的可信度与博弈合理性：

---

### 1. **谈判条件的变化是否符合常识**
- 允许合理的谈判条件变化（比如让步等），但是要符合博弈的逻辑和常识。重点关注让步方向是否符合谈判者身份所决定的正确让步方向/逻辑，让步的幅度不需要特别要求：
    - 正确的让步方向/逻辑：
        - 原告初始索赔 10万 → 让步后递降至 8万（符合“高索赔金额→低索赔金额”让步方向）。
        - 被告初始愿赔 5万 → 让步后递增至 6万（符合“低赔偿金额→高赔偿金额”让步方向）。 
    - 错误示例：
        - 被告上一轮愿赔 1万，本轮“让步”却降至 8000（实际并非让步）。
        - 原告已从 10万 让步至 8万，却无故加码至 9万。
- 是否对对方的新条件、新提议毫无回应，只是机械重复自己原有立场，缺乏回应与互动？

### 2. **数值与逻辑准确性**
- 是否存在金额、比例、时间等数值表达错误或计算矛盾？

### 3. **上下文连贯性与立场一致性**
- **你刚才的发言**是否与你自己此前的表态明显矛盾，或脱离上下文？
- 是否未回应他人或调解员的最新发言，导致对话不连贯？

### 4. **表达方式的人类真实感**
- **你刚才的发言**是否与**你的历史发言**的用词过度重复，缺乏语言变化？
- 是否在表达相同立场时未能使用不同的方式（如事实补充、情绪描述、条件引入）？
- 是否像“复制粘贴”而非自然对话？

### 5. **内容实质性与现实推进价值**
- **你刚才的发言**是否在现实博弈中边际收益极低的细节上反复纠缠，不符合真实调解行为特征？
    - 边际收益低的细节表现：金额差异几百元、赔偿比例百分之几等；
- **你刚才的发言**是否缺乏新增信息或态度调整，只是重复旧话？

---

注意：
- 注意，可以合理让步，且让步不需要特别详细的理由或解释，只要符合**谈判者身份所决定的正确让步方向/逻辑**即可；
- 可重复立场，但不得机械重复语言用词或句式结构 ，要体现语言自然变化；
- 允许情绪化表达（如愤怒、无奈、失望等），但需体现人类在交流中的思维动态与语言节奏 ，如犹豫、叹息、强调、反问等；
- 你的任务是提升发言在真实调解场景中的可信度、博弈策略合理性和互动推动力；
- 作为原告，合理的博弈逻辑是逐步降低索赔金额；
- 作为被告，合理的博弈逻辑是逐步提高赔偿金额；

你是{self.name}。请你对刚刚生成的发言内容进行**自我反思和评估**。

请一定要按照以下json格式输出, 可以被Python json.loads函数解析:（不要添加多余解释）：

```json
{{
    "角色身份（博弈逻辑）": "当前角色在博弈中是原告方还是被告方？合理的博弈逻辑是？",
    "分析过程": "简要说明分析过程和理由",
    "是否存在严重问题": "是/否",
